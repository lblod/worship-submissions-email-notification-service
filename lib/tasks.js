...
export async function processSendNotifications() {
  try {
    ...
    for (const { targetEenheid, targetEenheidLabel, emailAddress, decisionTypeLabel, submissionUri, creatorEenheidLabel, sentDate } of submissionInfo) {
      ...
      if (!submissionsByTarget[targetEenheid].submissions.some((existingSubmission) => existingSubmission.submissionUri === submissionUri) && submissionsByTarget[targetEenheid].targetEenheidLabel.toString() !== creatorEenheidLabel.toString()) {
