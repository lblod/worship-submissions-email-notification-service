...
export function generateHtmlEmail(targetEenheidLabel, submissions) {
  ...
  const htmlSubmissionsRows = submissions.map(({decisionTypeLabel, creatorEenheidLabel, sentDate, url}) => (
      `<tr style="border-bottom: .1rem solid #cfd5dd;">
        <td style="padding: .5rem;">Nieuwe inzending - ${decisionTypeLabel} aangemaakt door ${creatorEenheidLabel} op ${formatDate(sentDate)}</td>
        <td style="padding: .5rem; border-left: .1rem solid #cfd5dd;"><a href="${url}" target="_blank">Bekijk</a></td>
      </tr>`
    )).join('');
  ...
  <body>
    ...
    <th style="padding: 1rem; border: .1rem solid #cfd5dd;">Details voor inzending</th>
    <th style="padding: 1rem; border: .1rem solid #cfd5dd;"></th>
    ${htmlSubmissionsRows}
    </table>
    <br />
    <br />
