...
export function generateHtmlEmail(targetEenheidLabel, submissionUrls) {
  ...
  <body>
    ...
    <p style="margin:0; font-family:sans-serif; letter-spacing:normal; line-height:1.6;">Uw bestuur &quot;${targetEenheidLabel}&quot; heeft een nieuwe inzending ontvangen via het <a href="https://loket.lokaalbestuur.vlaanderen.be/login" target="_blank">Loket voor Lokale Besturen</a>, module Databank Erediensten.</p><br />
    ${htmlSubmissionLink}
...
export async function processSendNotifications() {
  try {
    ...
    for (const queryData of submissionInfo) {
      const { targetEenheid, targetEenheidLabel, emailAddress, decisionTypeLabel, submission } = queryData;
    ...
    for (const target in submissionsByTarget) {
      const { targetEenheidLabel, emailAddress, decisionTypeLabel, submissions } = submissionsByTarget[target];
      ...
      const subject =
        submissions.length > 1
          ...
          : `${decisionTypeLabel} - Nieuwe inzending`;
