import { uuid } from "mu";
import { getSubmissionInfo, insertEmail, getRelevantBestuurseenheden } from "../utils/queries";
...
export async function processSendNotifications() {
  try {
    ...
    for (const eenheid of bestuurseenheden) {
      ...
      await processSendNotificationForEenheid(eenheid, maxMessageAge);
    }
  } catch (err) {
    throw err;
  }
...
async function processSendNotificationForEenheid(eenheid, maxMessageAge) {
  try {
    ...
    await sendNotificationEmail(targetInfo, filteredSubmissions);
  } catch (err) {
    console.error(`Had an error for sending email to ${eenheid}`);
    console.error(err);
  }
}
