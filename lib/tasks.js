...
export async function processSendNotifications() {
  ...
  const emails = [];
  const errors = [];
  for (const [index, eenheid] of bestuurseenheden.entries()) {
    console.log(`Processing ${index + 1}/${bestuurseenheden.length}`);
    const { email, error } = await processSendNotificationForEenheid(eenheid, maxMessageAge);
    if(email)
      emails.push(email);
    if(error)
      errors.push(error);
  ...
  if(errors.length > 0) {
    const errorMsg = `
      There were ${errors.length} errors creating emails.
      Here is the first one of the list:

      ${errors[0]}

      You might want to check the logs.
    `;
    await createError(errorMsg);
  }
}

async function processSendNotificationForEenheid(eenheid, maxMessageAge) {
  const processedData = { email: null, error: null };
  try {
    ...
    processedData.email =  sendNotificationEmail(targetInfo, filteredSubmissions);
